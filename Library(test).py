...
def create():
    ...
    l.execute('CREATE TABLE IF NOT EXISTS Issue(Uid BLOB, book1 TEXT, book2 TEXT, book3 TEXT)')
...
def check_book(title, uid):
    ...
    if a is not None:
        if b is not None:
            # l.execute('SELECT book1 FROM Issue WHERE Uid=?', uid)
            # if l.fetchone() is None:
            #     l.execute('UPDATE Issue SET book1=? WHERE Uid=?', (title, uid))
            #     li.commit()
            #     return
            l.execute('SELECT book2 FROM Issue WHERE Uid=?', uid)
            print(l.fetchone())
            if l.fetchone() is None:
                l.execute('UPDATE Issue SET book2=? WHERE Uid=?', (title, uid))
                li.commit()
                return
            l.execute('SELECT book3 FROM Issue WHERE Uid=?', uid)
            c1 = l.fetchone()
            if c1 is None:
                l.execute('UPDATE Issue SET book2=? WHERE Uid=?', (title, uid))
                li.commit()
...
class User:
    ...
    def __init__(self, name, uid, key):
        ...
        l.execute("INSERT INTO Issue(Uid) VALUES(?)", (uid,))
        li.commit()
